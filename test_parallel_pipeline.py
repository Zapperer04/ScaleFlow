...
def run_test():
    print("--- Starting Stage 4: Parallel Pipeline Test ---")
    payload = {
        "name": "Stage 4 Parallel Pipeline",
        "pipeline_type": "test_parallel_pipeline",
        "initial_payload": {
    ...
    while time.time() - start_time < time_limit:
        ...
        if status in ["completed", "failed", "cancelled"]:
            if status == "completed":
                ...
                print(f"Stage 4: FAILED with status {status}")
                sys.exit(1)
                
        time.sleep(1)
        
    print("Stage 4: FAILED (Timeout)")
    sys.exit(1)
