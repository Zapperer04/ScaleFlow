...
from engine.document_pipeline.parser import VLMParser
from engine.document_pipeline.normalizer import CanonicalNormalizer
from engine.document_pipeline.storage import DocumentStore
from engine.document_pipeline.registry import DocumentRegistry, BuilderRegistry
...
class ProductionParsingOrchestrator:
    ...
    def rebuild_representations(self, document_id: str, targets: List[str] = None, force: bool = False, trace_fn = None) -> str:
        ...
        from engine.document_pipeline.schemas.definitions import CanonicalBlock, CanonicalTable, CanonicalEntity
        
        blocks = []
        for b in doc_dict.get("blocks", []):
            bbox = None
            if b.get("bbox"):
                from engine.document_pipeline.schemas import BoundingBox
            ...
            if name not in builder_names:
                ...
                if name == "graph":
                    ...
                    if chunks is not None:
                        ...
                        from engine.document_pipeline.schemas import SemanticChunk
