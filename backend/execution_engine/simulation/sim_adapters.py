...
class SimulatedGeminiAdapter(ResourceProvider):
...
class SimulatedOpenRouterAdapter(ResourceProvider):
    def get_provider_id(self) -> str:
        return "openrouter"

    def parse(self, artifact: ArtifactRef, prompt_payload: dict) -> Dict[str, Any]:
        url = os.environ.get("OPENROUTER_MOCK_URL", "")
        if url:
            res = requests.post(f"{url}/parse", json={"artifact": artifact.artifact_id, "prompt": prompt_payload, "streaming": False})
            if res.status_code == 429:
                raise Exception("429 Rate Limit Exceeded")
            ...
            try:
                ...
                raise Exception("Malformed JSON")
        return {"raw_provider_ast": "openrouter_output"}
